...
function saveCarReg() {
    if (selectedRow.value) {
        PermitService.updatePermit(selectedRow.value.id, carRegistration.value)
            .then(() => {
                selectedRow.value.carregistration = carRegistration.value;
                // Import Toast service at the top of script section

                // In the saveCarReg function, add this toast notification
                toast.add({
                    severity: 'success',
                    summary: 'Úspěch',
...
<template>
    <div>
        <div class="card">
            ...
            <DataTable ref="dt" v-model:selection="selectedProducts" :value="products" dataKey="id" :rows="10">
                <template #header>
                ...
                <Column :exportable="false" style="min-width: 12rem">
                    <template #body="slotProps">
                        <Button class="mr-2" @click="editProduct(slotProps.data)"
        ...
        <Dialog v-model:visible="showCarRegDialog" header="Změnit poznávací značku" :modal="true" :closable="true">
            <div class="flex flex-col gap-4">
                <Form :validation-schema="validationSchema" v-slot="{ errors }" @submit="saveCarReg" class="card">
                    <div class="grid grid-cols-12 gap-2">
                        <label for="name3" class="flex items-center col-span-12 mb-2 md:col-span-2 md:mb-0">Nová poznávací značka</label>
